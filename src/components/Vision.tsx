...
const Vision = () => {
  ...
  useEffect(() => {
    const observer = new IntersectionObserver(
      ([entry]) => {
        if (entry.isIntersecting && imageContainerRef.current) {
          imageContainerRef.current.classList.add('in-view');
        }
      },
      { threshold: 0.1 }
    );
  ...
  return (
    <section ref={sectionRef} id="vision" className="py-24 bg-white">
